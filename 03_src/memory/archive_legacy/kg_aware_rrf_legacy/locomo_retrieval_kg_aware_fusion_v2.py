...
CONFIGS = [
    {
        "config_name": "KG-aware-RRF-C-bge",
        "rrf_k": 60,
        "w_bm25": 1.0,
        "w_dense": 0.5,
        "w_fusion": 1.0,
...
INPUTS = {
    "bm25": {
        "path": str(BASE / "locomo_bm25_results.csv"),
        "columns": ["retrieved_memory_ids", "bm25_top10_memory_ids"],
    },
    "dense": {
        "path": str(BASE / "locomo_dense_bge_results.csv"),
        "columns": ["retrieved_memory_ids", "dense_bge_top10_memory_ids"],
    },
    "fusion": {
        "path": str(BASE / "locomo_fusion_bm25_dense_bge_results.csv"),
        "columns": ["fusion_top10_memory_ids", "retrieved_memory_ids"],
    },
    "kg": {
        "path": str(BASE / "locomo_cassandra_kg_results.csv"),
        "columns": ["retrieved_memory_ids", "kg_top10_memory_ids"],
    },
}

QA_CSV = str(BASE / "locomo_qa_records.csv")
EVIDENCE_CSV = str(BASE / "locomo_evidence_map.csv")
OUT_RESULTS = str(BASE / "locomo_kg_aware_fusion_bge_results.csv")
OUT_SUMMARY = str(BASE / "locomo_kg_aware_fusion_bge_summary.csv")
...
def pick_column(fieldnames, candidates, label, path):
    for c in candidates:
        if c in fieldnames:
            return c
    raise ValueError(f"No column found for {label} in {path}. Available: {fieldnames}")
...
def load_ranking_file(label, spec):
    ...
    print(f"{label}: loaded {len(out)} QA rankings from {spec['path']} (col={col})")
    return out, col
...
def main():
    ...
    for cfg in CONFIGS:
        ...
        for qa in qa_rows:
            ...
            row = {
                ...
                "dense_bge_top10_memory_ids": ";".join(dense_ids),
                "bm25_dense_bge_rrf_top10_memory_ids": ";".join(fusion_ids),
    ...
    result_fields = [
        ...
        "bm25_dense_bge_rrf_top10_memory_ids", "kg_boost_top10_memory_ids",
    ...
    for cfg in CONFIGS:
        ...
        summary_rows.append(summarize_group("KG-aware-RRF-bge", cfg, "ALL", rows))
        by_cat = defaultdict(list)
        for r in rows:
            by_cat[str(r["category"])].append(r)
        for cat in sorted(by_cat.keys(), key=lambda x: int(x) if x.isdigit() else 999):
            summary_rows.append(summarize_group("KG-aware-RRF-bge", cfg, f"cat_{cat}", by_cat[cat]))
    ...
    print("")
    print("=== KG-aware-RRF-bge Fusion Retrieval ===")
    print(f"QA count: {len(qa_rows)}")
    print("")
    print(f"{'Config':24s} {'R@1':>8s} {'R@5':>8s} {'R@10':>8s} {'MRR@10':>8s}")
    print("-" * 56)
    for r in all_summary:
        print(f"{r['config_name']:24s} {float(r['recall_1']):8.4f} {float(r['recall_5']):8.4f} {float(r['recall_10']):8.4f} {float(r['mrr_10']):8.4f}")
