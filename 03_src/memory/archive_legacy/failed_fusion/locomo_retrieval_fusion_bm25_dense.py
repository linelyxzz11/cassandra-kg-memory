...
def main():
    bm25 = {r["qa_id"]: r for r in load_csv_rows("results/locomo_bm25_results.csv")}
    dense = {r["qa_id"]: r for r in load_csv_rows("results/locomo_dense_onnx_results.csv")}
    qa_rows = load_csv_rows("results/locomo_qa_records.csv")

    gold_map = defaultdict(set)
    for r in load_csv_rows("results/locomo_evidence_map.csv"):
    ...
    for qa in qa_rows:
        ...
        results.append({
            ...
            "dense_top10_memory_ids": ";".join(dense_ranked[:10]),
    ...
    r_fields = ["qa_id", "category", "question", "gold_memory_ids",
                "bm25_top10_memory_ids", "dense_top10_memory_ids",
                "fusion_top1_memory_ids", "fusion_top5_memory_ids",
                "fusion_top10_memory_ids", "recall_1", "recall_5",
                "recall_10", "rr_10"]
    with Path("results/locomo_fusion_bm25_dense_results.csv").open("w", encoding="utf-8", newline="") as f:
    ...
    print(f"=== BM25 + Dense Fusion Results ===")
    print(f"Method: RRF(BM25, Dense ONNX), k={rrf_k}")
    ...
    summary_rows = []
    all_r = sum(r["recall_1"] for r in results) / len(results)
    all_r5 = sum(r["recall_5"] for r in results) / len(results)
    all_r10 = sum(r["recall_10"] for r in results) / len(results)
    all_mrr = sum(r["rr_10"] for r in results) / len(results)
    summary_rows.append({
        "method": "RRF(BM25, Dense ONNX)",
        "category": "ALL", "n": len(results),
        "recall_1": f"{all_r:.4f}", "recall_5": f"{all_r5:.4f}",
        "recall_10": f"{all_r10:.4f}", "mrr_10": f"{all_mrr:.4f}",
    })
    ...
    for cat in sorted(by_cat.keys()):
        ...
        summary_rows.append({
            "method": "RRF(BM25, Dense ONNX)", "category": f"cat_{cat}",
            "n": n, "recall_1": f"{cr1:.4f}", "recall_5": f"{cr5:.4f}",
            "recall_10": f"{cr10:.4f}", "mrr_10": f"{cmrr:.4f}",
        })

    s_fields = ["method", "category", "n", "recall_1", "recall_5", "recall_10", "mrr_10"]
    with Path("results/locomo_fusion_bm25_dense_summary.csv").open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=s_fields)
        w.writeheader()
        w.writerows(summary_rows)

    print(f"\nOutput: results/locomo_fusion_bm25_dense_results.csv")
    print(f"Summary: results/locomo_fusion_bm25_dense_summary.csv")
