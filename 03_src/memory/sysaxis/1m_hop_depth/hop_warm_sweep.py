"""1M hop-depth warm sweep: 40 trials (2 sys x 4 hops x 5 reps). Read-only. 15s warmup + 45s measure."""
...
def run_trial(sysname, hop, rep):
    ...
    tag = f"{sysname} hop={hop} r={rep+1} warm"
    print(f"\n  [{tag}]", flush=True)
    queries = [json.loads(line) for line in open(manifest_path)]

    def client_loop(ci, per_lat, per_rt, per_rw, per_err, stop):
        ridx = ci % len(queries)
    ...
    # Phase 1: warmup 15s
    per_lat_w = [[] for _ in range(CLIENTS)]; per_err_w = [0] * CLIENTS
    per_rt_w = [[] for _ in range(CLIENTS)]; per_rw_w = [[] for _ in range(CLIENTS)]
    stop_w = threading.Event()
    threads_w = [threading.Thread(target=client_loop, args=(i, per_lat_w, per_rt_w, per_rw_w, per_err_w, stop_w), daemon=True) for i in range(CLIENTS)]
    for t in threads_w: t.start()
    print(f"    warmup 15s ...", flush=True)
    time.sleep(15)
    stop_w.set()
    for t in threads_w: t.join(timeout=10)
    w_reads = sum(len(l) for l in per_lat_w)

    # Phase 2: measurement 45s
    per_lat = [[] for _ in range(CLIENTS)]; per_err = [0] * CLIENTS
    per_rt = [[] for _ in range(CLIENTS)]; per_rw = [[] for _ in range(CLIENTS)]
    stop = threading.Event()
    threads = [threading.Thread(target=client_loop, args=(i, per_lat, per_rt, per_rw, per_err, stop), daemon=True) for i in range(CLIENTS)]
    ...
    row = {
        "run_id": DATE_TAG, "system": sysname, "hop": hop, "mode": "warm", "clients": CLIENTS,
    ...
    print(f"    warm_reads={w_reads} reads={nr} QPS={row['QPS']:.0f} mean={row['mean_ms']:.1f}ms p95={row['p95_ms']:.1f}ms spot={read_ok} err={errs}", flush=True)
    return row
...
# -- Main --
print("=== 1M HOP-DEPTH WARM SWEEP ===")
print("Systems: cassandra_opt, neo4j | Hops: 1-4 x5 | Warmup 15s + Measure 45s | Total: 40 warm trials")
...
jls_path = OUT / "trial_summary_warm.jsonl"
spot_path = OUT / "correctness_spotcheck_warm.jsonl"
fail_path = OUT / "failures_warm.jsonl"
for fn in [csv_path, jls_path, spot_path, fail_path]: fn.unlink(missing_ok=True)
...
for hop in [1, 2, 3, 4]:
    ...
    with (OUT / f"hash_gate_after_warm_cassandra_h{hop}.json").open("w") as f: json.dump(hc, f, indent=2)
    with (OUT / f"hash_gate_after_warm_neo4j_h{hop}.json").open("w") as f: json.dump(hn, f, indent=2)
...
with (OUT / "final_hop_depth_summary_warm.csv").open("w", newline="", encoding="utf-8-sig") as f:
    w = csv.DictWriter(f, fieldnames=ff, extrasaction="ignore"); w.writeheader(); w.writerows(final_warm)
with (OUT / "final_hop_depth_summary_warm.json").open("w") as f: json.dump(final_warm, f, indent=2)

print(f"\n=== FINAL WARM ===")
for fr in final_warm:
...
# Cold+Warm combined summary
cold_csv = OUT / "final_hop_depth_summary_cold.csv"
cff = list(ff)
cff.insert(0, "mode")
cold_rows = []
with cold_csv.open(encoding="utf-8-sig") as f:
    for r in csv.DictReader(f):
        r = dict(r); r["mode"] = "cold"; cold_rows.append(r)
warm_rows = []
for fr in final_warm:
    fr = dict(fr); fr["mode"] = "warm"; warm_rows.append(fr)
combined = cold_rows + warm_rows
cw_csv = OUT / "final_hop_depth_summary_cold_warm.csv"
with cw_csv.open("w", newline="", encoding="utf-8-sig") as f:
    w = csv.DictWriter(f, fieldnames=cff, extrasaction="ignore"); w.writeheader(); w.writerows(combined)
with (OUT / "final_hop_depth_summary_cold_warm.json").open("w") as f: json.dump(combined, f, indent=2)

# Warmup efficiency summary
print(f"\n=== WARMUP EFFICIENCY ===")
cold_rows_dict = defaultdict(dict)
for r in csv.DictReader(cold_csv.open(encoding="utf-8-sig")):
    cold_rows_dict[(r["system"], int(r["hop"]))] = float(r["median_mean_ms"])
for fr in final_warm:
    key = (fr["system"], fr["hop"])
    cold_ms = cold_rows_dict.get(key)
    warm_ms = fr["median_mean_ms"]
    if cold_ms:
        delta = cold_ms - warm_ms
        pct = (delta / cold_ms) * 100
        print(f"  {fr['system']:15s} hop={fr['hop']} cold={cold_ms:.1f}ms warm={warm_ms:.1f}ms delta={delta:.1f}ms ({pct:+.1f}%)")
