"""1M concurrency warm sweep — add warmup=15s, otherwise same as cold."""
...
def run_trial(sysname, clients, rep):
    ...
    for l in per_lat: l.clear()  # reset after warmup
...
# ── Main ──
print("=== 1M CONCURRENCY WARM SWEEP ===")
print("Systems: cassandra_opt, cassandra_naive, neo4j | Clients: 1/8/32/64 x5")
print("Warmup=15s Measure=45s | Total: 60 warm trials")
...
csv_path = OUT/"trial_summary_warm.csv"; jls_path = OUT/"trial_summary_warm.jsonl"
spot_path = OUT/"correctness_spotcheck_warm.jsonl"; fail_path = OUT/"failures_warm.jsonl"
