"""
SysAxis 1M warm write-ratio sweep. 30 trials (2 sys × 3 wr × 5 reps).
...
def run_trial(system, wr_pct, repeat_idx):
    wgid = f"sysaxis1m_write_sink_{system}_w{wr_pct}_r{repeat_idx+1}_warm_{DATE_TAG}"
    tag = f"{system} wr={wr_pct}% r={repeat_idx+1} warm"
    print(f"\n  [{tag}]", flush=True)
    ...
    row = {
        ...
        "repeat": repeat_idx+1, "mode": "warm", "warm_mode": "process_warm",
        "warmup_seconds": 15, "measurement_seconds": 45.0,
        ...
        "relation_index_enabled": False, "backend_state": "process_warm",
...
def main():
    ...
    csv_path = OUT_DIR / "trial_summary_warm.csv"
    jls_path = OUT_DIR / "trial_summary_warm.jsonl"
    spot_path = OUT_DIR / "correctness_spotcheck_warm.jsonl"
    wval_path = OUT_DIR / "write_validation_warm.jsonl"
    fail_path = OUT_DIR / "failures_warm.jsonl"
    ...
    print("[1] Read graph guard after warm...", flush=True)
    guard_after, guard_ok = read_guard()
    with (OUT_DIR/"read_graph_guard_after_warm.json").open("w") as f: json.dump(guard_after, f, indent=2)
    print("[2] Full 256 hash gate...", flush=True)
    hg_after = full_hash_gate()
    with (OUT_DIR/"hash_gate_after_warm.json").open("w") as f: json.dump(hg_after, f, indent=2)
    ...
    with (OUT_DIR/"final_write_ratio_summary_warm.csv").open("w", newline="", encoding="utf-8-sig") as f:
        w = csv.DictWriter(f, fieldnames=ff, extrasaction="ignore"); w.writeheader(); w.writerows(final)
    with (OUT_DIR/"final_write_ratio_summary_warm.json").open("w") as f: json.dump(final, f, indent=2)

    with fail_path.open("w") as f: pass  # ensure exists

    print(f"\n{'='*50}")
    print("WARM SWEEP SUMMARY")
    for fr in final:
