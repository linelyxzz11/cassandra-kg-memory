...
from cassmem.retrieval.online import ScopedOnlineRetrievalIndex, render_rawerk


ROOT = Path(__file__).resolve().parents[2]
DEFAULT_OUTPUT = ROOT / "results" / "serving_100k" / "locomo_workload_preflight"
...
def main() -> None:
    ...
    memories = read_csv(ROOT / "data" / "locomo_memory_records.csv")
    features = {row["memory_id"]: row for row in read_csv(ROOT / "data" / "frozen_retrieval" / "p3_memory_features.csv")}
    eligible = read_csv(ROOT / "results" / "serving_100k" / "locomo_workload_profile" / "freshness_eligible_cat1_4.csv")
