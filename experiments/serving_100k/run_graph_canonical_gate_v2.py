...
from cassmem.representation.graph_event import GraphRecord, digest
...
def build_graph_records() -> list[GraphRecord]:
    memories = csv_rows(ROOT / "data" / "locomo_memory_records.csv")
    features = {row["memory_id"]: row for row in csv_rows(ROOT / "data" / "frozen_retrieval" / "p3_memory_features.csv")}
    ids = (ROOT / "data" / "locomo_memory_ids_bge.txt").read_text(encoding="utf-8-sig").splitlines()
    vectors = np.load(ROOT / "data" / "locomo_memory_bge_large.npy", mmap_mode="r")
    vector_hashes = {
