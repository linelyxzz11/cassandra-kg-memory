...
def main():
    parser = argparse.ArgumentParser(
        description="Cassandra-KG enhanced retrieval: TF-IDF + KG boost."
    )
    ...
    parser.add_argument("--output", default="results/locomo_cassandra_kg_results.csv")
    parser.add_argument("--top-k", type=int, default=10)
    parser.add_argument("--kg-boost", type=float, default=0.5,
                        help="Bonus weight for KG-backed memories")
    ...
    print(f"  {len(kg_set)} KG edges loaded")
    ...
    print(f"\nRunning {len(qa_rows)} queries (TF-IDF + KG boost={args.kg_boost})...")
    ...
    print(f"\n=== Final Results ===")
    print(f"Method: TF-IDF + Cassandra-KG boost (boost={args.kg_boost})")
    print(f"KG coverage: {100*kg_coverage/n_memories:.1f}% of memory turns")
