...
S(q,i)
&=0.6\,\bar z_d(q,i)+0.4\,\bar z_l(q,i),\\
\mathcal O_k(q)
...
其中，$\bar z_c$ 对已召回候选沿用 $z_c$，对该通道未召回的候选取其最低标准化分数；空通道取 0。这使仅被一路发现的记忆仍可参与排序，无需扩展召回范围来补算另一路分数。融合分数 $S$ 汇合两类证据的相对支持程度，并输出 Top-10 记忆供后续读取。
