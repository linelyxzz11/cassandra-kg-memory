...
foreach ($rep in @(0,1,2)) {
    foreach ($concurrency in @(1,8,16,32,64)) {
        foreach ($cell in $cells) {
            $summary = "05_reports\locomo_workload_graph_v2_100k\stage_sidecar\${cell}_c${concurrency}_r${rep}_summary.json"
            if (-not (Test-Path -LiteralPath $summary)) {
                & $pythonExe '04_experiments\locomo_workload\run_graph_95_5_stage_sidecar_v2.py' --cell $cell --concurrency $concurrency --rep $rep
