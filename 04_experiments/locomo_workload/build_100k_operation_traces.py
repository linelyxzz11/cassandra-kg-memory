...
ROOT=Path(__file__).resolve().parents[2]; OUT=ROOT/'05_reports'/'locomo_workload_100k'/'traces';SEED=20260803
def read(p):
 with p.open(encoding='utf-8-sig',newline='') as f:return list(csv.DictReader(f))
def digest(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def build(rep):
 qa=read(ROOT/'01_data/locomo_qa_records.csv'); eligible=read(ROOT/'05_reports/locomo_workload_profile/freshness_eligible_cat1_4.csv')
 reads=[];updates=[]
 for r in range(17):
  pre=f'lr{r:06d}::'
