...
def main():
    ...
    qa_rows = []
    with (ROOT / "01_data" / "locomo_qa_records.csv").open(encoding="utf-8-sig", newline="") as handle:
    ...
    memory_ids = (ROOT / "01_data" / "locomo_memory_ids_bge.txt").read_text(encoding="utf-8-sig").splitlines(); memory_array = np.load(ROOT / "01_data" / "locomo_memory_bge_large.npy", mmap_mode="r"); memory_vector = {memory_id: memory_array[index] for index,memory_id in enumerate(memory_ids)}
    qa_ids = (ROOT / "01_data" / "locomo_qa_ids_bge.txt").read_text(encoding="utf-8-sig").splitlines(); qa_array = np.load(ROOT / "01_data" / "locomo_qa_bge_large.npy", mmap_mode="r"); qa_vector = {qa_id: qa_array[index] for index,qa_id in enumerate(qa_ids)}
